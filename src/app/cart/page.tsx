                            ...
                            {(() => {
                                ...
                                return Object.values(groupedItems).map((grouped, index) => {
                                    ...
                                    return (
                                        <div
                                            key={item.id}
                                            className="group bg-white p-4 md:p-5 rounded-2xl flex flex-col sm:flex-row items-start gap-4 border border-primary/5 hover:border-primary/20 transition-all duration-500 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_40px_-15px_rgba(140,43,238,0.1)] hover:-translate-y-0.5"
                                        >
                                            <div
                                                className="relative w-20 h-20 md:w-24 md:h-24 rounded-xl overflow-hidden bg-slate-50 cursor-pointer flex-shrink-0"
                                            ...
                                            <div className="flex-1 w-full min-w-0">
                                                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 w-full">
                                                    <div>
                                                        <h3 className="font-black text-lg md:text-xl text-background-dark truncate font-brand italic tracking-tight leading-none mb-0.5">{item.name}</h3>
                                                    ...
                                                    <div className="sm:text-right flex items-center sm:block gap-3">
                                                        <p className="text-lg md:text-xl font-black text-background-dark leading-none">
                                                            {currency === 'USD' ? `$${groupTotal.toFixed(2)}` : `${groupTotal.toFixed(2)} Bs`}
                                                        </p>
                                                        {isWholesale && <span className="text-[8px] bg-green-500 text-white px-2 py-0.5 rounded-full font-black uppercase inline-block mt-0 sm:mt-1">Mayorista</span>}
                                                    </div>
                                                </div>

                                                {/* Wholesale Progress Bar */}
                                                {item.wholesaleMin && item.wholesaleMin > 1 && (
                                                    <div className="mt-2 w-full max-w-xs bg-slate-50 p-2 rounded-lg border border-slate-100">
                                                        <div className="flex justify-between items-center mb-1">
                                                            <span className="text-[8px] font-black uppercase text-primary/60 tracking-wider">Progreso Mayorista</span>
                                                        ...
                                                        {!isWholesale && (
                                                            <p className="text-[8px] font-bold text-slate-500 mt-1.5 leading-tight">
                                                                {item.wholesaleMin && item.requiredTonesCount ? (
                                                                    <span>Faltan <span className="text-primary">{item.wholesaleMin - totalQtyForProduct}</span> uds. o min. <span className="text-primary">{item.requiredTonesCount}</span> tonos para dto</span>
                                                                ) : item.wholesaleMin ? (
                                                ...
                                                <div className="mt-3 grid gap-1.5">
                                                    {variants.map(variant => (
                                                        <div key={variant.selectedColor || 'base'} className="flex flex-wrap sm:flex-nowrap items-center justify-between gap-2 bg-white p-2 rounded-lg border border-primary/10 shadow-sm">
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                <div className="flex items-center bg-slate-50 rounded-md p-0.5 border border-slate-200">
                                                                    <button onClick={(e) => { e.stopPropagation(); handleQuantityChange(variant.id, variant.quantity, -1, variant.selectedColor); }} className="p-0.5 hover:text-primary transition-colors"><Minus className="w-2.5 h-2.5" /></button>
                                                                    <span className="px-2 font-black text-[11px] min-w-[16px] text-center text-background-dark">{variant.quantity}</span>
                                                                    <button onClick={(e) => { e.stopPropagation(); handleQuantityChange(variant.id, variant.quantity, 1, variant.selectedColor); }} className="p-0.5 hover:text-primary transition-colors"><Plus className="w-2.5 h-2.5" /></button>
                                                                </div>
                                                                <button onClick={(e) => { e.stopPropagation(); removeItem(variant.id, variant.selectedColor); }} className="text-slate-400 hover:text-red-500 hover:bg-red-50 p-1.5 rounded-md transition-all" title="Eliminar tono">
                                                ...
                                                {item.colors && item.colors.length > 0 && (
                                                    <button 
                                                        ...
                                                        className="mt-2 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase text-primary bg-primary/5 hover:bg-primary/10 transition-colors inline-flex items-center gap-1.5 border border-primary/10"
                                                    >
